		...
		if($mail->send()) {
			$msg="Call Letter has been sent successfully";
					$type="success";
							...
							if($rnum==1)$run = "UPDATE 2interview_detail SET 1date='$date1', 1time='$time1', 1type_interview='$type_interview1' ,1attach_jd='$attach_jd1', 1type_jd='$type_jd1', 1interviwer_name='$interviwer_name1', call_letter='yes',venue='$venue' WHERE fk = '$id' AND c_id=".$_SESSION['c_id'];
						 else
						 {
						 $run="INSERT Into 2interview_detail (fk,c_id,1date,1time,1type_interview,1attach_jd,1type_jd,call_letter,1interviwer_name,venue) values('$id','$c_id','$date1','$time1','$type_interview1','$attach_jd1','$type_jd1','yes','$interviwer_name1','$venue')";
						 	}
		...
		else
		 {
					...
					$type="fail";
						  ...
						  if($rnum==1)$run = "UPDATE 2interview_detail SET 1date='$date1', 1time='$time1', 1type_interview='$type_interview1' ,1attach_jd='$attach_jd1', 1type_jd='$type_jd1', 1interviwer_name='$interviwer_name1', call_letter='no',venue='$venue' WHERE fk = '$id' AND c_id=".$_SESSION['c_id'];
						 else
						 {
						 	$run="INSERT Into 2interview_detail (fk,c_id,1date,1time,1type_interview,1attach_jd,1type_jd,call_letter,1interviwer_name,venue) values('$id','$c_id','$date1','$time1','$type_interview1','$attach_jd1','$type_jd1','no','$interviwer_name1','$venue' )";
						 	}
			 $result=mysqli_query($db,$run);
...
if ($rnum==1)
 ...
 else
 {
     ...
     $INSERT = "INSERT Into 2interview_detail (fk,c_id,1date, 1time,1type_interview,venue,1attach_jd, 1type_jd ,1appeared_interview, 1shortlisted,1interviwer_name,1mark_final_round,1next_round,1rejection_reason,1cooling_period,1blacklist,1reason_non_appearance) values(?,?,?, ?, ?, ?, ?, ?,? ,?,?, ?, ?, ?, ? ,?,?)";
